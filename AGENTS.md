...
This repo mirrors to a **public** GitHub repo (`trtmn/agent-plugins`) — anything committed here is public, including full history. Never commit `/Users/<name>/` paths (use `~/`, `$HOME/`, or `$CLAUDE_PLUGIN_ROOT`), personal hostnames, Tailscale MagicDNS names, SSH usernames, real machine names, vault names, 1Password item UUIDs, or real email addresses — in code, SKILL.md/agent/command prose, **design docs under `docs/`**, and test fixtures alike. Use placeholders: `example.com`, `octocat`, `jane@example.com`, `<vault>`, `<item>`, `<broker-host>`, `machine-a`.

This has actually happened: a personal Tailscale hostname, SSH username, and real machine names leaked into git history via `docs/superpowers/specs/` and `plugins/side-quest/` (design doc prose, a hardcoded default, and test fixture data all carried the same real names) and required a `git filter-repo` rewrite across every branch — main plus four feature branches — to remove. `docs/` is not exempt just because it's not `plugins/`; it gets committed and pushed the same as everything else.

**If a script needs a piece of personal infra** (an MQTT broker host, a specific server) — require it via an env var with **no hardcoded personal default**. Fail loudly or degrade gracefully (`${VAR:?must be set}` in shell, `os.environ.get("VAR")` returning `None` and the feature no-opping in Python) rather than silently falling back to your own infrastructure. See `plugins/yt-dlp/skills/yt-dlp/scripts/ytdlp.py` (`YTDLP_REMOTE_HOST`/`YTDLP_REMOTE_PATH`) for the pattern: unset means "skip this feature," never "use my server."

**Before committing anything** (new files, edits, or docs), run this sweep over what's about to be committed and inspect every hit — most are false positives (CGNAT-range examples like `100.64.0.0/10`, docs illustrating the `op://<vault>/<item>/field` syntax, RFC 5737 example IPs), but each one needs a look, not a reflexive dismissal:

```bash
git diff --cached --name-only -z | xargs -0 grep -nIE \
  '/Users/[A-Za-z0-9_.-]+|@[A-Za-z0-9.-]+\.ts\.net|op://[A-Za-z0-9_-]+/[a-z0-9]{20,}|[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}' \
  2>/dev/null | grep -viE 'example\.(com|local)|octocat|jane@'
```

For anything that pattern can't catch mechanically — a real hostname, a real person's or machine's name used as sample data — there's no substitute for reading the diff. When in doubt about whether a name is real personal infrastructure or a fictional example, ask rather than assume.

If PII does slip into a commit that gets pushed, a working-tree fix is not enough — the old value is still live in history on the public mirror. It needs a `git filter-repo` rewrite (blob content *and* commit messages — `--replace-text` does not touch commit messages by default, a `--message-callback` pass is needed too) across every branch that shares the history, followed by a force-push to every remote and every branch that has it, not just `main`.

## Security

Never commit sensitive information to this repository. This includes, but is not limited to, API keys, secrets, credentials, private certificates, or any personally identifiable information (see PII Prevention above for specifics on this repo's recurring failure mode and the mandatory pre-commit sweep). Always use environment variables, 1Password `op://` references, or other secret management to handle sensitive data required by skills or scripts.
